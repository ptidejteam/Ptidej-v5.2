package org.tigris.giant.gefworkbench;
...
public class ZoomSliderButton extends PopupButton {
    ...
    public ZoomSliderButton() {
        super();
        Icon icon = ResourceLoader.lookupIconResource("Zoom");
        setIcon(icon);
        setToolTipText("Zoom");
    }
    ...
    private void createPopupComponent() {
        ...
        for (Enumeration e = slider.getLabelTable().elements(); 
            e.hasMoreElements();) {
            ((Component) e.nextElement()).setFont(LABEL_FONT);
        }
        
//        _slider.setToolTipText(Translator.localize(BUNDLE, 
//            "button.zoom.slider-tooltip"));   
        slider.setToolTipText("button.zoom.slider-tooltip");   
        ...
        currentValue.setToolTipText("button.zoom.current-zoom-magnification");
